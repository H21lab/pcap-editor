...
def source_code(val, payload_var=None):
    ...
    if payload_var:
        payload_inject = f"""
     # Inject payload into NAS-PDU IE (id=26)
     (lambda: [
         (lambda ies: [
             (ie.__setitem__('value', ('NAS-PDU', bytes({payload_var}))) if ie.get('id') == 26 else None)
             for ie in ies
    ...
    [S1AP := __import__('pycrate_asn1dir.S1AP', fromlist=['S1AP']),
