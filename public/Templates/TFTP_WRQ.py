"""TFTP_WRQ template using proper Scapy encoding."""
import binascii
import copy
from scapy.layers.tftp import TFTP, TFTP_WRQ
...
def decode(hex_data, **kwargs):
    ...
    try:
        ...
        # Get the WRQ layer if present
        if pkt.haslayer(TFTP_WRQ):
            wrq = pkt[TFTP_WRQ]
            for f in wrq.fields_desc:
                v = wrq.getfieldval(f.name)
...
def encode(val):
    ...
    try:
        ...
        filename = val_copy.get('filename', '')
        mode = val_copy.get('mode', 'octet')
        ...
        # TFTP(op=2) / TFTP_WRQ creates full packet with opcode
        pkt = TFTP(op=2) / TFTP_WRQ(filename=filename, mode=mode)
