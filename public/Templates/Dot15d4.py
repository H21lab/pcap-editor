...
def encode(val):
    ...
    # If not edited, always return original hex to ensure 100% match
    if orig_hex and not is_edited:
        return orig_hex
        
    raw_payload = _find_and_remove_raw(val_copy)
    try:
        pkt = Dot15d4(**val_copy)
        if raw_payload: pkt.add_payload(raw_payload)
        return binascii.hexlify(bytes(pkt)).decode()
    except: return ""

def source_code(val, payload_var):
    fields = [f"{k}={safe_repr(v)}" for k, v in val.items() if k not in ['raw', 'load', 'payload', '_orig_hex', '_edited']]
    ctor = f"Dot15d4({', '.join(fields)})"
